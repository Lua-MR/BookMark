...
const LibraryScreen = ({ books, searchQuery, setIsModalOpen, addNewBook, onUpdateBook, onDeleteBook }) => {
  ...
  const [newBook, setNewBook] = useState({ name: '', author: '', genre: '', totalPages: 0, currentPage: 0 });
  ...
  const sortBooks = books => {
    ...
    return [...books].sort((a, b) => {
      switch (sortCriteria) {
        ...
        case 'progress': return (a.currentPage / a.totalPages) - (b.currentPage / b.totalPages);
        case 'status': return a.status.localeCompare(b.status);
        case 'genre': return a.genre.localeCompare(b.genre);
        case 'totalPages': return a.totalPages - b.totalPages;
        case 'currentPage': return a.currentPage - b.currentPage;
        case 'publisher': return a.publisher.localeCompare(b.publisher);
        case 'isbn': return a.isbn.localeCompare(b.isbn);
        case 'type': return a.type.localeCompare(b.type);
        case 'property': return a.prop.localeCompare(b.prop);
        case 'file': return a.file ? 1 : -1;
  ...
  const enrichedBooks = sortedBooks.map((book) => {
    ...
    return { ...book, goalName: bookGoal ? bookGoal.name : "Sem meta definida" };
  });
  ...
  const addNewTab = () => {
    if (newTabName) {
      const newTab = { id: getNextTabId(), name: newTabName, type: newTabType, fields: selectedFields };
  ...
  const removeTab = (tabId) => {
    ...
    if (tabToRemove && (tabToRemove.name === 'galeria' || tabToRemove.name === 'lista')) {
  ...
  const defaultListFields = [
    'imagem', 'nome', 'autor', 'genero', 'totalPaginas', 'paginaAtual', 
    'editora', 'isbn', 'tipo', 'propriedade', 'arquivo', 'avaliacao', 
    'status', 'meta', 'resenha', 'dataInicio', 'dataTermino'
  ];
  
  return (
    <div className="library-screen">
      <div className="tabs">
        <button className={`tab-button ${activeTab === 'galeria' ? 'active-tab' : ''}`} onClick={() => changeActiveTab('galeria')}>
          Galeria
        </button>
        <button className={`tab-button ${activeTab === 'lista' ? 'active-tab' : ''}`} onClick={() => changeActiveTab('lista')}>
          Lista
        </button>
        {customTabs.map(tab => (
          <button 
          ...
          >
            ...
            {tab.name}
            <span className="close-list" onClick={(e) => { e.stopPropagation(); removeTab(tab.id); }}>X</span>
          </button>
        ))}
        <button className="add-tab-button" onClick={openConfigModal}>+</button>
      </div>
      ...
      <div className="tab-content">
        {activeTab === 'galeria' && <BookGallery books={sortedBooks} onBookClick={handleBookClick} />}
        {activeTab === 'lista' && (
          <BookList
            books={sortedBooks}
          ...
          if (tab.id === activeTab) {
            return (
              <div key={tab.id} className="custom-tab-content">
                {tab.type === 'card' ? (
                  <BookGallery books={sortedBooks} fields={tab.fields} onBookClick={handleBookClick} />
                ) : (
                  <BookList
                    books={sortedBooks}
                    fields={tab.fields || []}
      ...
      {isConfigModalOpen && (
        <div className="modal">
          <div className="modal-content-list">
            <button className="modal-close-list" onClick={closeConfigModal}>X</button>
            <label>
              Nome da Aba:
            ...
            <div className="fields-container">
              <div className="fields-column">
                {['imagem', 'nome', 'autor', 'genero', 'totalPaginas', 'paginaAtual', 'dataInicio', 'dataTermino'].map(field => (
                  <label key={field}>
                    <input
                      type="checkbox"
                      value={field}
                    ...
                    />
                    {field === 'dataInicio' ? 'Data de Início' : field === 'dataTermino' ? 'Data de Término' : field.charAt(0).toUpperCase() + field.slice(1)}
                  </label>
                ))}
              </div>
              <div className="fields-column">
                {['editora', 'isbn', 'tipo', 'propriedade', 'arquivo', 'avaliacao'].map(field => (
                  <label key={field}>
                    <input
                      type="checkbox"
                      value={field}
